...
public class SynchronousFuzzyPetriExecutor {
  ...
  private IFuzzyPetriBehaviourRecorder recorder;
  ...
  public void putInitialZeroTokenInPlaceZero() {
    this.stateOfPlaces.set(0, FuzzyToken.zeroToken());
  }

  private void intilazeOrder() {
    orderOfTransition = new ArrayList<>();
    List<Integer> trWithInputs = IntStream.range(0, myNet.getNrOfTransition()).filter(trID -> myNet
        .getPlacesNeededForTransition(trID).stream().filter(plId -> myNet.isInputPlace(plId)).findAny().isPresent())
    ...
    List<Integer> trWithoutDelays = IntStream.range(0, myNet.getNrOfTransition())
        .filter(trId -> !orderOfTransition.contains(trId)).filter(trId -> myNet.getDelayForTransition(trId) == 0)
        .boxed().collect(toList());
    ...
    List<Integer> trWithDelays = IntStream.range(0, myNet.getNrOfTransition())
        .filter(trId -> !orderOfTransition.contains(trId)).filter(trId -> myNet.getDelayForTransition(trId) != 0)
        .boxed()
        .sorted((trId, trId2) -> Integer.compare(myNet.getDelayForTransition(trId), myNet.getDelayForTransition(trId2)))
        .collect(toList());
    orderOfTransition.addAll(trWithDelays);

  }

  public void startTick(Map<Integer, FuzzyToken> inputs) {

    setInputPlacesWithToken(inputs);
    updateDelayTransitionsAllreadyInFire();
  ...
  private void updateDelayTransitionsAllreadyInFire() {
  ...
  private void finsihFire(int trId) {
    ...
    if (nrOfNeededOut != res.length) {
      throw new RuntimeException("Ucorrect Petri Structure at trastition " + trId + ", ouputs needed" + nrOfNeededOut
          + " and exists " + res.length);
    }
