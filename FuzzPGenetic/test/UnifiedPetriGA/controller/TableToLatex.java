...
public class TableToLatex {
  ...
  public static void saveTabeLatex(UnifiedTwoXOneTable table, String filename, boolean withPhi) {
    StringBuilder bld = new StringBuilder();
    Map<FuzzyValue, Map<FuzzyValue, FuzzyValue>> first = table.getTable();
    for (FuzzyValue fvOne : (withPhi) ? FuzzyValue.inOrder : FuzzyValue.inOrderWithoutPhi) {
      for (FuzzyValue fvTwo : (withPhi) ? FuzzyValue.inOrder : FuzzyValue.inOrderWithoutPhi) {
        ...
        bld.append("& $ ").append(createStr(ll1)).append(" $");
      }
      bld.append("\\\\ \\hline\n");
    }
    writeToFile(filename, bld);
  ...
  private static void writeToFile(String fileName, StringBuilder bld) {
    try {
