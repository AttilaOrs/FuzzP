...
public class FuzzyPetrinetBehaviourModel {
  ...
  private void buildChacheForPlace(int placeId) {
    ...
    for (int tickNr = 0; tickNr < eventsGroupedByTick.size(); tickNr++) {
      ArrayList<Double> dataInCurrentTick = new ArrayList<>();
      ...
      if (!dataInCurrentTick.isEmpty()) {
        double step = 1.0 / (dataInCurrentTick.size() + 1.0);
        for (int dataIndex = 0; dataIndex < dataInCurrentTick.size(); dataIndex++) {
          table.add(tickNr + step * (dataIndex + 1), dataInCurrentTick.get(dataIndex));
        }
      }
